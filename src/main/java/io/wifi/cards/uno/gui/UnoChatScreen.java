...
/**
 * 牌桌界面内的聊天屏（纯客户端）：原版聊天框的子类。
 * <p>原版 ChatScreen 在 Esc / 发送消息后会直接 <code>setScreen(null)</code>（回到游戏 HUD），
 * 这里覆盖 keyPressed 的 Esc/Enter 分支，关掉聊天框后回到打开前的牌桌界面；
 * 背景渲染牌桌界面（聊天历史由本类自行渲染，避免与牌桌界面常驻聊天区重复）。</p>
 */
public class UnoChatScreen extends ChatScreen {
    private final Screen parent;

    public UnoChatScreen(Screen parent) {
        super("");
        this.parent = parent;
    }
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        if (parent != null) {
            // 背景：牌桌界面（聊天历史由下方 ChatComponent 渲染，牌桌界面本身不绘制聊天区）
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        Minecraft mc = Minecraft.getInstance();
        if (keyCode == 256) { // Esc：原版直接 setScreen(null)，这里回到牌桌界面
            mc.setScreen(parent);
            return true;
        }
        if (keyCode == 257 || keyCode == 335) { // Enter：原版发送后 setScreen(null)，这里保持牌桌界面
